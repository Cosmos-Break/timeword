    ...
    for line in data:
        ...
        for word in words:
            if "/t" not in word:
                ...
                length = len(word)
                if length == 1:
                    crfFormat.append(word[0] + " " + "O" + "\n")
                elif length == 2:
                    crfFormat.append(word[0] + " " + "TB" + "\n")
                    crfFormat.append(word[1] + " " + "TE" + "\n")
                elif length >= 3:  # length >= 3
                    crfFormat.append(word[0] + " " + "TB" + "\n")
                    for i in range(1, length - 1):
                        crfFormat.append(word[i] + " " + "TM" + "\n")
                    crfFormat.append(word[length - 1] + " " + "TE" + "\n")
...
f = open("test.char.bmes", 'w', encoding='utf-8', newline='\n')
for a in crfTestSet:
    f.write(a)
f.close()
f = open("train.char.bmes", 'w', encoding='utf-8', newline='\n')
for a in crfTrainSet:
    f.write(a)
f.close()
f = open("dev.char.bmes", 'w', encoding='utf-8', newline='\n')
for a in crfDevSet:
    f.write(a)
f.close()
